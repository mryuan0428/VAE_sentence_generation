...
#==========加载目录和文档==========#
'''
首先，我们将设置主目录和一些有关文本特征的变量。
将序列长度设置为5-20，将词汇表中的最大单词数设置为270000(原270054)，我们将使用200维embeddings。
最后，从txt加载文本。文本文件来源于新闻、网页、小说等，包含大约287万个句子(130万条符合长度要求)。
'''
BASE_DIR = './data/'
TRAIN_DATA_FILE = BASE_DIR + 'train.txt'# 287+万条句子
GLOVE_EMBEDDING = BASE_DIR + 'Tencent_AILab_ChineseEmbedding.txt'#单词->200维embedding
VALIDATION_SPLIT = 0.2
MIN_SEQUENCE_LENGTH = 5  #最小序列长度5
MAX_SEQUENCE_LENGTH = 20 #最大序列长度20
MAX_NB_WORDS = 270000
EMBEDDING_DIM = 200 #embedding维度200
...
x = Input(shape=(max_len,)) #输入是按批量的20维向量(句子)
...
checkpointer = create_model_checkpoint('models', 'vae_seq2seq_test_very_high_std_MD001')
...
vae.save('models/BiLSTM-VAE-MD001.h5')
...
sentence1=['gogogo 我们 要 努力 建设 中国特色 和谐 社会 endend']
...
sentence2=['gogogo 我们 应该 努力 建设 新 家园 endend']
